__all__ = ["ResultsWriter", "get_monitor_files", "load_results"]
...
class CustomMonitor(gym.Wrapper):
    ...
    def step(self, action: Union[np.ndarray, int]) -> GymStepReturn:
        ...
        if done:
            ...
            ep_info = {"r": round(ep_rew, 6), "l": ep_len, "t": round(time.time() - self.t_start, 6), "context": self.env.unwrapped._task.goals[0].pos}
...
class ResultsWriter:
    ...
    def __init__(
        ...
        if header is None:
            header = {}
        if not filename.endswith(CustomMonitor.EXT):
            if os.path.isdir(filename):
                filename = os.path.join(filename, CustomMonitor.EXT)
            else:
                filename = filename + "." + CustomMonitor.EXT
...
def get_monitor_files(path: str) -> List[str]:
    ...
    """
    return glob(os.path.join(path, "*" + CustomMonitor.EXT))
...
def load_results(path: str) -> pandas.DataFrame:
    ...
    if len(monitor_files) == 0:
        raise LoadMonitorResultsError(f"No monitor files of the form *{CustomMonitor.EXT} found in {path}")
    data_frames, headers = [], []
