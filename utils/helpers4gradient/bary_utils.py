import ot
from utils.helpers4gradient.w_encode import *
...
class GRADIENT(object):
    ...
    def compute_barycenter_l2(self,
        ...
        if visualize:
            ...
            for i in range(n_alphas):
                ...
                ax.scatter(x=bary_centers[i][:, 0], y=bary_centers[i][:, 1], color=rgb, edgecolor='k',
                           label='Barycenter', alpha=0.6)
            ax.scatter(x=bary_center_return[:, 0], y=bary_center_return[:, 1], color='yellow', edgecolor='k',
                       label='Highlighted')
            ax.set(xlim=[-6, 14], ylim=[-6, 14])
            ax.set_aspect('equal', 'box')
            plt.savefig(fig_save_path)
    ...
    def train_CAE_and_compute_barycenter(
        ...
        bary_center_return_decoded = self.cae.decoder(
            self.cae.numpy_to_cuda(bary_center_return_encoded)).cpu().detach().numpy()
        ...
        if visualize:
            ...
            for i in range(n_alphas):
                ...
                ax.scatter(x=bary_centers[i][:, 0], y=bary_centers[i][:, 1], color=rgb, edgecolor='k',
                           label='Barycenter', alpha=0.6)
            ax.scatter(x=bary_center_return_decoded[:, 0], y=bary_center_return_decoded[:, 1], color='yellow',
                       edgecolor='k', label='Highlighted')
            ax.set(xlim=[-6, 14], ylim=[-6, 14])
            ax.set_aspect('equal', 'box')
            plt.savefig(fig_save_path)
            plt.close()
